...
public class SearchDoctorByPageForm {
    ...
    @Pattern(regexp = "^本科$|^研究生$|^博士$", message = "degree 内容不正确")
    ...
    @Range(min = 1, max = 3, message = "status 内容不正确")
    private Byte status;

    @Pattern(regexp = "^ASC$|^DESC$", message = "order 内容不正确")
    ...
    @Range(min = 10, max = 50, message = "length 内容不正确")
